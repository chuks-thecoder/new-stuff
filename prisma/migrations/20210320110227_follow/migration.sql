/*
  ...
  - You are about to drop the column `profileId` on the `followers` table. All the data in the column will be lost.
  - You are about to drop the column `profileId` on the `following` table. All the data in the column will be lost.
...
-- AlterTable
ALTER TABLE `followers` DROP COLUMN `profileId`,
    ADD COLUMN     `folprofileId` INT;

-- AlterTable
ALTER TABLE `following` DROP COLUMN `profileId`,
    ADD COLUMN     `folprofileId` INT;
...
-- AddForeignKey
ALTER TABLE `Followers` ADD FOREIGN KEY (`folprofileId`) REFERENCES `Profile`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `Following` ADD FOREIGN KEY (`folprofileId`) REFERENCES `Profile`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
